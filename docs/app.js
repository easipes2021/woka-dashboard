...
console.log("App.js: Refactored dashboard initializing...");
...
const CONFIG = {
  ...
  sskpCurve: {
    ...
    /**
     * This continuity adjustment prevents the upper branch from dropping
     * below the lower branch at the breakpoint.
     *
     * If you later replace this with a validated field-calibrated curve,
     * you can set continuityAdjust = false and use your raw formula.
     */
...
function ratingCurveCFS(gageHeightFt) {
  ...
  // Continuity-adjust the upper branch so it does not "drop" at the breakpoint
...
function getThemeColors() {
  ...
  return {
    ...
    lakeLine: "#007bff",
    lakeFill: "rgba(0, 123, 255, 0.12)",
    flowLine: "#28a745",
    flowFill: "rgba(40, 167, 69, 0.12)"
  };
}
...
function toFlowChartData(values) {
  return values
    .map((v) => ({
      x: new Date(v.dateTime),
      y: safeNumber(v.value, NaN)
